---
name: regen-ai
description: Regenerate the AI-facing derived artifacts of chartjs2img (docs/public/llms.txt, docs/public/llms-full.txt) from the SSOT in src/llm-docs/ and docs/en/, and validate the Agent Skills under plugins/chartjs2img/. Use after editing the Chart.js / plugin LLM docs, the built-in examples, the HTML template, the CLI usage banner, any English guide page, or any SKILL.md - or before merging a branch that touches any of those.
...
3. **Validate the Agent Skills bundle.** Runs unconditionally — the
   skill frontmatter, plugin.json version, and discovery keywords are
   all part of the AI-facing surface:
   ```bash
   bun run validate-plugin-skills
   ```

4. **If the user touched `src/examples.ts` or `src/template.ts`**, also
...
5. **Verify** the code still compiles and docs still build:
   ```bash
...
6. **Summarize** for the user:
   ...
   - Any validator or build warnings surfaced
...
- Do **not** hand-edit a generated file to paper over a failure. See `.claude/rules/ai-artifacts-policy.md` — fix the source (LLM docs, example config, guide markdown, SKILL.md) and rerun `/regen-ai`.
