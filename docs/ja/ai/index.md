...
既定の `skia` エンジンはブラウザ不要でインプロセス描画するため、
何も準備は要りません。実ブラウザとのピクセル一致が必要で `browser`
エンジンを使う場合のみ、Chromium はレンダー時に chartjs2img 自身が
自動で扱います。linux-arm64 の注意事項はメインのインストールガイドを
参照。
