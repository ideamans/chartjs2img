...
主な用途はレンダリング時間の調整と、`browser` エンジン利用時の
Chromium の場所指定です（`CHROMIUM_PATH` / `PLAYWRIGHT_BROWSERS_PATH`
は既定の `skia` エンジンでは無関係）。
...
| 変数                        | 既定値      | 説明                                                                                         |
| --------------------------- | ----------- | -------------------------------------------------------------------------------------------- |
| `CHROMIUM_PATH`             | *(なし)*    | (`browser` エンジンのみ) Chromium / Chrome バイナリへの明示パス。自動検出より優先されます。   |
| `PLAYWRIGHT_BROWSERS_PATH`  | *(なし)*    | (`browser` エンジンのみ) 検出時にスキャンする Playwright キャッシュディレクトリを上書きします。 |
...
`browser` エンジンで、自動検出が見つけられない Chromium を指定する
必要がある場合に使います（既定の `skia` エンジンでは不要）。典型例は
Linux ARM64 で、Chrome for Testing の自動ダウンロードが linux-arm64
向けに提供されていないためです。
