...
2. **Chart.js コンソールメッセージ** — レンダラ（`skia` はインプロセス、
   `browser` はヘッドレスブラウザ）からキャプチャして HTTP ヘッダで
   返します。
3. **サーバーレベルエラー** — 認証、入力検証（不正な `engine` 値を含む）、
   `browser` エンジンの Chromium 障害。通常の HTTP ステータスコードで
   表現します。
...
| `400`  | クライアント入力不備: `chart` 欠落、JSON ボディの構文エラー、`?chart=` クエリ不正、`engine` が `"skia"`/`"browser"` 以外など。 |
| `401`  | `API_KEY` 必要なのに未提示／誤り。[認証](./auth) 参照。                                              |
| `404`  | 不明なパス、期限切れ／存在しないキャッシュハッシュ。                                                  |
| `405`  | `/render` に対する許可されていないメソッド（例: `PUT`）。                                              |
| `500`  | サーバー内部エラー — `browser` エンジンの Chromium 起動失敗、ディスクフル、CDN ネットワーク障害など。 |
...
## Chromium レベルの障害（`browser` エンジン）

以下は `browser` エンジン利用時のみ発生します（既定の `skia` エンジンは
ブラウザを起動しません）。ブラウザ自体が起動しない（バイナリ欠落、
権限不足、共有メモリ不足のコンテナなど）場合、サーバーは `500` を次の
ようなメッセージとともに返します。
