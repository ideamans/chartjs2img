---
title: 同梱プラグイン
description: chartjs2img に同梱される 12 種類の Chart.js プラグイン - datalabels, annotation, zoom, gradient, treemap, matrix, sankey, wordcloud, geo, graph, venn, date-fns 日付アダプタ。
---

# 同梱プラグイン

プラグインのバンドルは [CLI](./cli/) と [HTTP サーバー](./http/) で
共通です（両者は同一のレンダラーを共有）。このページは両方のトラック
からリンクされる共通リファレンスです。

chartjs2img は Chart.js コア **+ 12 プラグインエコシステム** を両方の
レンダリングエンジンに同梱しています（既定の `skia` エンジンは npm から、
`browser` エンジンはページ内に CDN から読み込み）。インストール不要 —
設定 JSON でオプションを書くだけ。

::: warning `chartjs-plugin-zoom` は `skia` エンジンに含まれません
`chartjs-plugin-zoom` は操作専用で DOM を必要とするため、既定の `skia`
エンジンには**含まれません**。ズーム設定に依存する場合は `browser`
エンジンを使ってください。
:::
...
| プラグイン                                                                                  | バージョン | 用途                                       |
| ------------------------------------------------------------------------------------------- | --------- | ------------------------------------------ |
| [chartjs-adapter-date-fns](https://github.com/chartjs/chartjs-adapter-date-fns)              | 3.0.0     | 時間軸 (`scales.x.type: "time"`) に必須    |
...
- **アニメーション** — `options.animation` は内部で常に OFF。レンダラは
  ...
  半端なフレームで切り取られてしまいます。
- **持ち込みの自作プラグイン** — レンダラが実行するのは同梱される上記
  プラグインのみ。追加するには開発者ガイド
